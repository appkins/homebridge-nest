...
const nestLogin = async (userId, password) => {
  ...
  try {
    ...
    if (isPi()) {
      ...
      browser = await puppeteer.launch({ executablePath: '/usr/bin/chromium-browser', headless: true, args: ['--no-sandbox'] })
    } else {
