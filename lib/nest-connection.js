...
const nestLogin = require('./nest-auth.js');
...
class Connection {
    ...
    async auth(preemptive) {
        ...
        while (true) {
            ...
            if (this.config.googleAuth && this.config.googleAuth.password && this.config.googleAuth.username) {
                let res = await nestLogin(this.config.googleAuth.username, this.config.googleAuth.password);

                if (length(res.accessToken) > 6) {
                    if (res.accessToken.substr(0, 6) == 'Bearer') {
                        this.config.access_token = res.accessToken.substr(6);
                    }
                }
                // this.config.googleAuth.apiKey = res.apiKey;
            } else if (this.config.googleAuth || this.config.refreshToken) {
                    ...
                    translateProperty(object, 'peer_devices', null, (peerDevice, id) => {
                        ...
                        if (!peerDevice.object.id.startsWith('STRUCTURE_')) {
                            ...
                            peerDevice.data.property.value.devices.forEach(el => {
                                ...
                                } else if (['nest.resource.NestLearningThermostat3Resource', 'nest.resource.NestAgateDisplayResource', 'nest.resource.NestOnyxResource', 'google.resource.GoogleZirconium1Resource'].includes(deviceType)) {
